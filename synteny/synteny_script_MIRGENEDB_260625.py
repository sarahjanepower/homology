...
# 21 Oct 2025
# MIRGENEDB ONLY
# This script retrieves upstream and downstream 10 protein coding genes for a given input mir-XXX (argument 1)  within gff3 files which has been filtered to just contain CDS entries and miRNAs.
...
print(f"Extracting {mir_input}..")

mirnas_pattern = re.compile(rf'\b{re.escape(mir_input)}\b')

file_names = [f for f in os.listdir('.') if f.endswith('mirs_genomic_sorted.bed')]
...
for file_name in file_names:
    ...
    with open(file_name, "r") as f:
        ...
        for ln in filtered_lines:
            m = prefix_pattern.search(ln)
            if m:
                p = m.group(1)
                prefix_totals[p] = prefix_totals.get(p, 0) + 1
        ...
        for index, line in enumerate(filtered_lines):
            ...
            if mirnas_pattern.search(line):
                ...
                if prefix_totals.get(prefix, 0) > 1:
