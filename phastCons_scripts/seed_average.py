...
for mir in wig_long["miRNA_clean"].unique():
    ...
    region_means = {}
    start_col, end_col = "seed_start", "seed_end"

    if pd.notna(best[start_col]) and pd.notna(best[end_col]):
        start, end = int(best[start_col]), int(best[end_col])

        if best.get("mature_arm", "") == "5p":
            end += 1

            # Non-inclusive end
        seed_scores = subwig[(subwig["Pos_"] >= start) & (subwig["Pos_"] < end)]["Score"].tolist()

        # Take up to 7 scores
        for i in range(7):
            col_name = f"seed_score_{i + 1}"
            if i < len(seed_scores):
                region_means[col_name] = seed_scores[i]
            else:
                region_means[col_name] = np.nan

        # Also add the average
        region_means["seed_avg"] = np.mean(seed_scores) if seed_scores else np.nan
    else:
        # If no valid seed region, fill with NaNs
        for i in range(7):
            region_means[f"seed_score_{i + 1}"] = np.nan
        region_means["seed_avg"] = np.nan

    results.append({"miRNA": mir, **region_means})

avg_df = pd.DataFrame(results)
avg_df.to_csv("seed_avg_scores.tsv", sep="\t", index=False)
print("Saved seed_avg_scores.tsv (END_INCLUSIVE={})".format(END_INCLUSIVE))
